...
<head>
	...
	<script type="text/javascript">
		...
		//面包屑导航
		function setbreadcrumb(cour_num){
			$.ajax({  
        		url:"/HomeWorkManageSystem/manageServlet",//servlet文件的名称
        		type:"POST",
        		data:{"funct":"getUpdateC",
        			  "cour_num": cour_num },
        		async:false
        	});
		...
		function addCourseExcel(){
			$.ajax({  
        		url:"/HomeWorkManageSystem/manageServlet",//servlet文件的名称
        		type:"POST",
        		data:{"funct":"addCourseExcel"},
        		async:false
        	});
			window.location.reload();
		}
...
<body>
			...
			<c:if test="${!empty course}" > 
				<c:forEach items="${course}" var="c" varStatus="status">
					...
					<c:if test="${status.count%2!=0 }" >
						...
						<td>${ c.cour_t_ID }</td>
						<td>${ c.cour_Credit }</td>
						<td><a href="/HomeWorkManageSystem/manage/m_updatecourse.jsp" onclick="setbreadcrumb('${c.cour_Num }')">修改</a></td>
						<td>
							<a type="button" class="close" href = "javascript:del('${c.cour_Num }')">
	...
	<button class = "btn btn-info" onclick = "addCourseExcel()" style = "height:40px;width:78px;margin-left: 90%">导入 </button>
